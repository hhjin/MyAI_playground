...
You put index number of source URL at the place of the answer text which is related with the source URL. Only put the index number in answer main body. Only list the full source URL in the answer end. Never include URLs that are not in the CONTEXT sections. Never make up URLs.
""")
    ...
    Source_URLs: List[str] = Field(..., description="""You put index number of source URL at the place of the answer text which is related with the source URL. Only put the index number in answer main body. Only list the full source URL in the answer end. Never include URLs that are not in the CONTEXT sections. Never make up URLs.""")
...
def bttqa_chat_chain(llm, store, query , chat_prompt, topk=6 , rerank=False, funcall=False ):
    ...
    if funcall :   # function call 
        ...
        answer=response 
    save_chat_log_file(query, contexs_text, answer , "ic1666c-bgeEmb-bgeRank", "azure_chat" ,save_rootPath )
...
myQAKit=QA_Toolkit("LocalData/old_chroma/Chroma_DB_UDTT_IC1666c")
...
save_rootPath="LocalData/chatLogs/tmp" 
while True:
        query =input("\n\n####### Question for BTT: ")
        ...
        result=bttqa_chat_chain(llmAzureChat, storeBGE_Chroma, query, chat_prompt ,13,rerank=False ,funcall=True)               
'''

import json
breakedIndex=1476  # count from 1
